...
predicted_ages <- predict(epiLM, newdata = as.data.frame(epigeneticAge))
...
data_long
data_long$residual <- as.numeric(data_long$epigeneticAge) - as.numeric(data_long$age)
p <- ggplot(data_long, mapping = aes(residual, as.numeric(Proportion), colour = diagnosis)) + geom_point() + ylab('Cell Proportion')
p + facet_wrap(vars(CellType))

head(cellProp)
lm(as.numeric(cellProp$Astrocyte) ~ as.numeric(cellProp$age))
cellProp$Astrocyte
model <- lapply(split(data_long, data_long$CellType), function(subdata) {
  lm(Proportion ~ age, data = subdata)
})

model2 <- lapply(split(data_long, data_long$CellType), function(subdata) {
  lm(Proportion ~ age + epigeneticAge, data = subdata)
})
model2
model

anova_results <- lapply(cell_types, function(var) {
  # Extract the models for the current response variable
  model1 <- model[[var]]
  model22 <- model2[[var]]
  
  # Perform ANOVA
  result <- anova(model1, model22)
  return(data.frame(variable = var, result))
})
anova_summary <- do.call(rbind, anova_results)
filtered_summary <- anova_summary[!is.na(anova_summary$Df), ]
filtered_summary[filtered_summary$Pr..F. < 0.005, ]
filtered_summary
# remove outlier
data_long <- data_long[data_long$residual > -18, ]
model <- lapply(split(data_long, data_long$CellType), function(subdata) {
  lm(Proportion ~ age, data = subdata)
})
